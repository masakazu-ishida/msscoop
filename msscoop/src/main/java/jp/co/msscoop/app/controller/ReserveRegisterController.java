...
public class ReserveRegisterController {
	...
	public String input(ReserveRegisterForm registerForm , BindingResult result,  Model model) {
	...
	String confirm(@ModelAttribute("registerForm") ReserveRegisterForm registerForm, Model model) {
	...
	String commit(@ModelAttribute("registerForm") ReserveRegisterForm registerForm,RedirectAttributes redirectAttr) {
		
		String id  = reserveService.register(registerForm);
		
		//予約情報をリダイレクト先に伝える
		redirectAttr.addAttribute("reserveId", id);
		
		return "redirect:/reserve/register?complete";
	}
	
	@GetMapping(params = "complete")
	String complete(Model model, RedirectAttributes redirectAttr, SessionStatus status) {
		status.setComplete();
		
		model.addAttribute("reserveId",redirectAttr.getAttribute("reserveId"));
	...
	public ModelAndView handleException(UseCaseException e) {
